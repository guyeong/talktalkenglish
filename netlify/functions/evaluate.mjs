  ...
  try {
    return await Promise.race([
      ...
      new Promise((_, reject) => {
        timer = setTimeout(() => reject(new Error("Pronunciation evaluation timed out")), timeoutMs);
      }),
    ]);
  } finally {
    clearTimeout(timer);
...
export default async (request) => {
  ...
  try {
    ...
    if (!text) return json({ error: "Gemini가 발음 평가 결과를 반환하지 않았습니다." }, 502);
    const result = JSON.parse(text);
    return json({ ...result, model: MODEL, evaluatedAt: Date.now() });
  } catch (error) {
